...
**边界声明：**
- `opportunity_score` 属于 M3 的解释层输出，不是独立的二次裁决器。
- `is_opportunity` 与 `priority_level` 仍由 M3 的主判断职责决定。
- 评分卡用于解释判断、供 M4/回测/模拟盘/复盘消费，不应反向覆盖主判断结论。
...
### 重要边界
- 对 M4：评分卡可以影响动作模板选择，但不应让 M4 重新裁决“这是不是机会”。
- 对 M5：`kill_switch_signals` 可以作为预定义监控条件输入，但 M5 负责机械检查，不负责重新解释机会逻辑。
- 对 M6：评分卡主要用于事后检验哪些维度具有预测力。
