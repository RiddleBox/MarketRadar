...
## 评分维度定义（必须显式填写 opportunity_score）
每个维度 1-10 分，需基于信号证据给出，不可随意凑数：
- **catalyst_strength**: 催化剂强度。事件/变化是否具有实质性冲击？1=噪音，10=历史性事件
- **timeliness**: 时效性。机会窗口是否紧迫？1=无时间压力，10=窗口仅限今日
- **market_confirmation**: 市场确认度。价格/量能是否已开始反应？1=完全未反应，10=强确认
- **tradability**: 可交易性。有无流动性足够的标的？1=无可交易标的，10=高流动性ETF直接可操作
- **risk_clarity**: 风险边界清晰度。最坏情况是否可量化？1=风险完全未知，10=止损点明确可设
- **consensus_gap**: 预期差大小。市场是否尚未充分定价？1=完全price-in，10=市场完全忽视
- **signal_consistency**: 信号一致性。多条信号是否指向同一方向？1=严重矛盾，10=完全一致
- **overall_score**: 综合得分（0-10），应为以上维度的加权反映，非简单平均
...
- **极度恐惧（恐贪指数 ≤ 20）+ 利好/政策信号**：市场恐慌过度，机会优先级可适当提升，但需确认有结构性催化剂而非仅情绪修复
- **极度贪婪（恐贪指数 ≥ 80）+ 利好信号**：市场可能已过热，即使有利好也需警惕追高风险，优先级适当降低
- **情绪中性区间（恐贪指数 40-60）**：情绪信号对机会判断影响较小，以基本面和技术面信号为主
- 情绪信号不单独构成机会（单独的"市场恐惧"不是做多理由，必须配合结构性信号）
