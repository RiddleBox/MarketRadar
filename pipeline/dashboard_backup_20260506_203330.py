...
with tab_system:
    ...
    # ── 信号统计图表 ──
    st.subheader("📈 信号统计")
    signal_stats = load_signal_stats()  # 重新加载信号统计数据
    ...
    with c_left:
        ...
        if by_type:
            ...
            st.info("暂无信号数据")
    with c_right:
        batches = signal_stats.get("recent_batches", {})
        if batches:
            import pandas as pd
            df_b = pd.DataFrame(
    ...
    with fc3:
        ...
        if db_path.exists():
            ...
            st.caption(f"总信号: {signal_stats.get('total', 0)} 条")
        else:
...
with tab_control:
    st.header("🎛️ 工作流控制中心")
    st.write("DEBUG: 进入工作流控制中心代码块")  # 调试输出
    ...
    try:
        sched_state = load_scheduler_state()
        st.write(f"DEBUG: sched_state = {sched_state}")  # 调试输出
        is_running = sched_state.get("running", False)
    except Exception as e:
        st.error(f"加载调度器状态失败: {e}")
        sched_state = {}
        is_running = False
    ...
    with col_sched1:
        ...
        st.metric("调度器状态", f"{status_color} {'运行中' if is_running else '已停止'}")
    with col_sched2:
        tasks = sched_state.get("tasks", {})
    ...
    with col_sched3:
        ...
        success_count = sum(1 for r in recent_runs if r.get("status") == "ok")
        st.metric("最近成功率", f"{success_count}/{len(recent_runs)}" if recent_runs else "N/A")

    # 调度器控制按钮
    ctrl_col1, ctrl_col2, ctrl_col3 = st.columns(3)
    with ctrl_col1:
        if st.button("🚀 启动调度器", use_container_width=True, disabled=is_running):
            import subprocess
            try:
                subprocess.Popen(
                ...
                st.success("调度器启动命令已发送")
                st.info("请等待3-5秒后刷新页面查看状态")
            except Exception as e:
                st.error(f"启动失败: {e}")

    with ctrl_col2:
        if st.button("⏸️ 停止调度器", use_container_width=True, disabled=not is_running):
            import subprocess
            try:
                subprocess.run(
    ...
    st.markdown("""
    **两种核心功能：**
    - **M12 扫描**: 主动扫描市场价格 → 异动检测 → 反向溯因 → 趋势判断 → 机会生成
    ...
    with trigger_col2:
        ...
        st.markdown("**如何添加信号：**")
        st.markdown("1. 创建文本文件（.txt/.md）")
        st.markdown("2. 放入 `data/incoming/` 目录")
        st.markdown("3. 点击下方按钮处理")

        if st.button("▶ 执行信号处理", type="primary", use_container_width=True):
            import subprocess

            with st.spinner("正在处理信号文件..."):
                try:
                    result = subprocess.run(
